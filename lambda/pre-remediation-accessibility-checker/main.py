...
def download_file_from_s3(bucket_name, file_key, local_path, folder_prefix=''):
    s3 = boto3.client('s3')
    s3_path = f"pdf/{folder_prefix}{file_key}"
    print(f"Filename : {file_key} | File key in the function: {s3_path}")

    s3.download_file(bucket_name, s3_path, local_path)

    print(f"Filename : {file_key} | Downloaded {s3_path} from {bucket_name} to {local_path}")
...
def lambda_handler(event, context):
    ...
    print("folder_prefix:", folder_prefix)
    local_path = f"/tmp/{file_basename}"
    download_file_from_s3(s3_bucket, file_basename, local_path, folder_prefix)
